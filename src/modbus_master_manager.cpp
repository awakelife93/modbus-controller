...
void ModBusMasterManager::UpdateUnitId(uint8_t unitId) { mUnitId = unitId; }
...
void ModBusMasterManager::ReadDiscreteInputs(u_int16_t inputAddress,
                                             u_int16_t inputCount)
{
    masterTransaction(kReadDiscreteInputsCode, inputAddress, inputCount);
}

void ModBusMasterManager::WriteSingleCoil(u_int16_t coilAddress,
                                          u_int16_t coilValue)
{
    masterTransaction(kWriteSingleCoilCode, coilAddress, coilValue);
}

void ModBusMasterManager::WriteMultipleCoils(u_int16_t registerAddress,
                                             u_int16_t registerCount)
{
    masterTransaction(kWriteMultipleCoilsCode, registerAddress, registerCount);
}

void ModBusMasterManager::ReadHoldingRegisters(u_int16_t inputAddress,
                                               u_int16_t registerCount)
{
    masterTransaction(kReadHoldingRegistersCode, inputAddress, registerCount);
}

void ModBusMasterManager::ReadInputRegister(u_int16_t inputAddress,
                                            u_int16_t registerCount)
{
    masterTransaction(kReadInputRegisterCode, inputAddress, registerCount);
}

void ModBusMasterManager::WriteSingleRegister(u_int16_t registerAddress,
                                              u_int16_t registerValue)
{
    masterTransaction(kWriteSingleRegisterCode, registerAddress, registerValue);
}

void ModBusMasterManager::WriteMultipleRegisters(u_int16_t registerAddress,
                                                 u_int16_t registerCount)
{
    masterTransaction(kWriteMultipleRegistersCode, registerAddress,
                      registerCount);
}
